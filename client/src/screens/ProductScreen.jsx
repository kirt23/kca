...
function ProductScreen()  {
		...
		const addToCartHandler = async () => {
			...
			//   navigate('/signin?redirect=/shipping'); // Redirect to sign-in page
			//   return;
			// }
				...
				<div className="left-layout">
					<div className="top-layout">
						...
						<div className="product-info-layout">
							...
							<div style={{fontSize: '1.5rem', marginBottom: '10px', fontWeight: 'bold', color: '#888'}}><center>P {product.price}</center></div>
							<span className="add-to-cart"><i className="fas fa-cart-plus"
								style={{marginRight: '10px'}}/>{' '}ADD TO CART</span>
						</div>
					</div>
					<div className="bot-layout">
						{
							[product.image, ...product.images].map((x) => (
								<img src={x} alt={'product'} onClick={() => setSelectedImage(x)}/>
							))
						}
					</div>
				</div>
